...
// tolerance    
    tol=1;
// Inside texture thickness
    itt = 0.5;
// Alignment notches size (width = depth)
    ns = 1.5;
// $fn
    $fn=64;
...
union() {
    difference() {
        gland_bolt();
        rotate([0,0,0]) cut(180);
    }
    translate([0,0,1]) dove_tail(ns);
}

translate ([0,hd/2+2,0]) difference () {    
    intersection() {
        gland_bolt();
        rotate([0,0,0]) cut(180);
    }
    translate([0,0.5,1]) dove_tail(ns+tol/2);
}
...
module gland () {
    union(){
        difference(){
            union() {
                parte_externa();
                translate([0,0,eph]) parte_interna();
            }
            internal_hole();
        }
        inner_texture();
...
module gland_bolt () {
    union(){
        difference(){
            hex_screw(od,thread_step,step_shape_degrees,iph,resolution,countersink,hd,eph,non_thread_length,non_thread_diameter);
            internal_hole();
        }
        inner_texture();
    }
...
//    offset(r=1) {rotate([0,0,0])cylinder(d=hd, h=eph,$fn=6);}
    linear_extrude(height=eph) {
        offset(r=1) {rotate([0,0,0]) circle(d=hd-2, $fn=6);}
    }
}
   

module parte_ext_rdd() {
    intersection(){
        parte_externa();
        translate([0,0,0.9*hd]) sphere(d=2*hd);
    }
}

module cut(a) {
rotate_extrude(angle=a) {
    square([1.25*hd/2,eph+iph+2]);
}
}

module dove_tail (w=2) {
    linear_extrude(height=eph-1) {
    translate([-((hd/2-id/2)/2+id/2),0]) rotate([0,0,45]) circle(d=w, $fn=4);
    translate([(hd/2-id/2)/2+id/2,0]) rotate([0,0,45]) circle(d=w, $fn=4);
    }
}
module parte_interna() {
    hex_screw(od,tp,step_shape_degrees,iph,resolution,countersink,head_diameter,0,non_thread_length,non_thread_diameter);
}

module internal_hole () {
    translate([0,0,-1]) cylinder(h=eph+iph+2, d=id);
}

module nut (d) {
    hex_nut(od+5,nh,tp,nut_step_shape_degrees,d,nut_resolution);
}

module inner_texture () {
    for (h = [1:1.5:iph+eph-2]) {
        translate([0,0,h]) rotate_extrude (angle=360) {
        translate([id/2,0,0]) rotate([0,0,180])circle(d=itt, $fn=24);}
    }
}
